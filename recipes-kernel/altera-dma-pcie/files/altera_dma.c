...
ssize_t altera_dma_read(struct file *file, char __user *buf, size_t count,
			loff_t *pos)
{
...
ssize_t altera_dma_write(struct file *file, char __user *buf, size_t count,
			 loff_t *pos)
{
...
ssize_t altera_dma_rw(struct file *file, char __user *buf, size_t count,
		      loff_t *pos, int read)
{
...
ssize_t altera_dma_exec_cmd(struct dma_cmd *ucmd,
			    struct altera_pcie_dma_bookkeep *bk_ptr)
{
	...
	case ALTERA_CMD_MODIFY_NUM_DWORDS: {
		...
		bk_ptr->dma_status.write_time = 0;
		bk_ptr->dma_status.read_time = 0;
		bk_ptr->dma_status.simul_time = 0;
		break;
	}
	...
	case ALTERA_CMD_READ_STATUS: {
		bk_ptr->dma_status.length_transfer =
		...
		curr.altera_dma_num_dwords =
			bk_ptr->dma_status.altera_dma_num_dwords;
		curr.altera_dma_descriptor_num =
			bk_ptr->dma_status.altera_dma_descriptor_num;
		curr.offset = bk_ptr->dma_status.offset;
		curr.read_eplast_timeout =
			bk_ptr->dma_status.read_eplast_timeout;
		curr.write_eplast_timeout =
			bk_ptr->dma_status.write_eplast_timeout;
	...
	default:
		printk(KERN_DEBUG
		       "command issued from user space doesn't exist %d",
		       kcmd.cmd);
	}
	return 0;
}
...
int altera_dma_open(struct inode *inode, struct file *file)
{
	...
	bk_ptr = container_of(inode->i_cdev, struct altera_pcie_dma_bookkeep,
			      cdev);
...
static int __init init_chrdev(struct altera_pcie_dma_bookkeep *bk_ptr)
{
	...
	int result = alloc_chrdev_region(&bk_ptr->cdevno, dev_minor, 1,
					 ALTERA_DMA_DEVFILE);
...
*/
static int set_read_desc(struct dma_descriptor *rd_desc, dma_addr_t source,
			 u64 dest, u32 ctl_dma_len, u32 id)
{
...
static int set_write_desc(struct dma_descriptor *wr_desc, u64 source,
			  dma_addr_t dest, u32 ctl_dma_len, u32 id)
{
...
static int scan_bars(struct altera_pcie_dma_bookkeep *bk_ptr,
		     struct pci_dev *dev)
{
	int i;
	for (i = 0; i < ALTERA_DMA_BAR_NUM; i++) {
		...
		dev_info(&dev->dev,
			 "BAR[%d] 0x%08lx-0x%08lx flags 0x%08lx, length %d", i,
			 bar_start, bar_end, bar_flags,
			 (int)bk_ptr->bar_length[i]);
	}
	return 0;
}

static int init_rp_mem(u8 *rp_buffer_virt_addr, u32 num_dwords, u32 init_value,
		       u8 increment)
{
	u32 i = 0;
	u32 tmp_rand;
...
static int rp_ep_compare(u8 *virt_addr, struct altera_pcie_dma_bookkeep *bk_ptr,
			 u32 mem_byte_offset, u32 num_dwords)
{
	u32 i = 0;
	u32 rp_data = 0;
	u32 ep_data = 0;
	...
	for (i = 0; i < num_dwords; i++) {
		if (bk_ptr->dma_status.onchip)
			ep_data = ioread32((u32 *)(bk_ptr->bar[BAR] +
						   mem_byte_offset +
						   ONCHIP_MEM_BASE) +
					   i);
		else
		...
		if (ep_data != rp_data) {
			if (bk_ptr->dma_status.onchip) {
				printk(KERN_DEBUG
				       "%p: 0x%08x != %p: 0x%08x => Data mismatch",
				       (u64 *)((u32 *)virt_addr + i), rp_data,
				       (u32 *)(bk_ptr->bar[BAR] +
					       mem_byte_offset +
					       ONCHIP_MEM_BASE) +
					       i,
			...
			for (j = 0; j < 300; j++) {
				ep_tmp = ioread32((u32 *)(bk_ptr->bar[BAR] +
							  mem_byte_offset +
							  OFFCHIP_MEM_BASE) +
						  i + j);
				rp_data = *((u32 *)virt_addr + i + j);
				printk(KERN_DEBUG
				       "RP offset %08x = %08x, EP = %08x\n",
				       i + j, rp_data, ep_tmp);
			}
			return 1;
		} else {
...
static int init_ep_mem(struct altera_pcie_dma_bookkeep *bk_ptr,
		       u32 mem_byte_offset, u32 num_dwords, u32 init_value,
		       u8 increment)
{
	u32 i = 0;
	...
	for (i = 0; i < num_dwords; i++) {
		...
		get_random_bytes(&tmp_rand, sizeof(tmp_rand));
		iowrite32(cpu_to_le32(tmp_rand),
			  (u32 *)(bk_ptr->bar[BAR] + mem_byte_offset) + i);
		wmb();
	}
...
static int dma_test(struct altera_pcie_dma_bookkeep *bk_ptr,
		    struct pci_dev *dev)
{
	...
	memset(rp_wr_buffer_virt_addr, 0,
	       bk_ptr->dma_status.altera_dma_num_dwords * 4);
	init_rp_mem(rp_rd_buffer_virt_addr,
		    bk_ptr->dma_status.altera_dma_num_dwords, 0x00000000, 1);
	init_rp_mem(rp_wr_buffer_virt_addr,
		    bk_ptr->dma_status.altera_dma_num_dwords, 0x00000000, 1);
	...
	if (bk_ptr->dma_status.onchip)
		init_ep_mem(bk_ptr, (u64)ONCHIP_MEM_BASE,
			    bk_ptr->dma_status.altera_dma_num_dwords, 0x0, 1);
	else
		init_ep_mem(bk_ptr, (u64)OFFCHIP_MEM_BASE,
			    bk_ptr->dma_status.altera_dma_num_dwords, 0x0, 1);
	...
	if (bk_ptr->dma_status.run_read) {
		...
		last_id = ioread32((u32 *)(bk_ptr->bar[0] + DESC_CTRLLER_BASE +
					   ALTERA_LITE_DMA_RD_LAST_PTR));

		set_lite_table_header((struct lite_dma_header *)bk_ptr
					      ->lite_table_rd_cpu_virt_addr);
		wmb();
		for (i = 0;
		     i < 128 /*bk_ptr->dma_status.altera_dma_descriptor_num*/;
		     i++) {
			if (bk_ptr->dma_status.onchip)
				set_read_desc(
				...
				set_read_desc(
					&bk_ptr->lite_table_rd_cpu_virt_addr
						 ->descriptors[i],
					(dma_addr_t)rp_rd_buffer_bus_addr,
					(u64)OFFCHIP_MEM_BASE,
					bk_ptr->dma_status.altera_dma_num_dwords,
					i);
		}
		iowrite32((dma_addr_t)bk_ptr->lite_table_rd_bus_addr,
			  bk_ptr->bar[0] + DESC_CTRLLER_BASE +
				  ALTERA_LITE_DMA_RD_RC_LOW_SRC_ADDR);
		iowrite32(((dma_addr_t)bk_ptr->lite_table_rd_bus_addr) >> 32,
			  bk_ptr->bar[0] + DESC_CTRLLER_BASE +
				  ALTERA_LITE_DMA_RD_RC_HIGH_SRC_ADDR);

		if (last_id == 0xFF) {
			iowrite32(
				RD_CTRL_BUF_BASE_LOW,
				bk_ptr->bar[0] + DESC_CTRLLER_BASE +
					ALTERA_LITE_DMA_RD_CTLR_LOW_DEST_ADDR);
			iowrite32(
				RD_CTRL_BUF_BASE_HI,
				bk_ptr->bar[0] + DESC_CTRLLER_BASE +
					ALTERA_LITE_DMA_RD_CTRL_HIGH_DEST_ADDR);
		}
		wmb();
		...
		last_id =
			last_id + bk_ptr->dma_status.altera_dma_descriptor_num;

		if (last_id > 127) {
			last_id = last_id - 128;
			if ((bk_ptr->dma_status.altera_dma_descriptor_num >
			     1) &&
			    (last_id != 127))
				write_127 = 1;
		}
		...
		while (1) {
			if (bk_ptr->lite_table_rd_cpu_virt_addr->header
				    .flags[last_id]) {
				break;
			}
		...
		if (timeout == 0) {
			...
			if (rp_ep_compare(
				    rp_rd_buffer_virt_addr, bk_ptr, 0,
				    bk_ptr->dma_status.altera_dma_num_dwords)) {
				bk_ptr->dma_status.pass_read = 0;
			} else
	...
	if (bk_ptr->dma_status.run_write) {
		...
		while (1) {
			if (bk_ptr->lite_table_wr_cpu_virt_addr->header
				    .flags[last_id]) {
				break;
			}
		...
		if (timeout == 0) {
			...
			if (rp_ep_compare(
				    rp_wr_buffer_virt_addr, bk_ptr, 0,
				    bk_ptr->dma_status.altera_dma_num_dwords)) {
				bk_ptr->dma_status.pass_write = 0;
			} else
	...
	if (bk_ptr->dma_status.run_simul) {
		...
		w_write_127 = 0;
		r_last_id =
			ioread32((u32 *)(bk_ptr->bar[0] + DESC_CTRLLER_BASE +
					 ALTERA_LITE_DMA_RD_LAST_PTR));
		w_last_id =
			ioread32((u32 *)(bk_ptr->bar[0] + DESC_CTRLLER_BASE +
					 ALTERA_LITE_DMA_WR_LAST_PTR));
		...
		if (bk_ptr->dma_status.onchip)
			init_ep_mem(
				...
				/*bk_ptr->dma_status.altera_dma_num_dwords*4 + */
				(u64)ONCHIP_MEM_BASE,
				bk_ptr->dma_status.altera_dma_num_dwords, 0x0,
				1);
		else
			init_ep_mem(
				bk_ptr,
		...
		if (r_write_127)
			iowrite32(127, bk_ptr->bar[0] + DESC_CTRLLER_BASE +
					       ALTERA_LITE_DMA_RD_LAST_PTR);
		iowrite32(r_last_id, bk_ptr->bar[0] + DESC_CTRLLER_BASE +
					     ALTERA_LITE_DMA_RD_LAST_PTR);

		if (w_write_127)
			iowrite32(127, bk_ptr->bar[0] + DESC_CTRLLER_BASE +
					       ALTERA_LITE_DMA_WR_LAST_PTR);
		iowrite32(w_last_id, bk_ptr->bar[0] + DESC_CTRLLER_BASE +
					     ALTERA_LITE_DMA_WR_LAST_PTR);

		while (1) {
			if ((bk_ptr->lite_table_wr_cpu_virt_addr->header
				     .flags[w_last_id]) &
			    (bk_ptr->lite_table_rd_cpu_virt_addr->header
				     .flags[r_last_id])) {
				break;
			}
		...
		if (timeout == 0) {
			...
			if (rp_ep_compare(
				    (u8 *)rp_rd_buffer_virt_addr, bk_ptr, 0,
				    bk_ptr->dma_status.altera_dma_num_dwords) ||
			    rp_ep_compare(
				    (u8 *)rp_wr_buffer_virt_addr, bk_ptr,
				    bk_ptr->dma_status.altera_dma_num_dwords *
					    4,
				    bk_ptr->dma_status.altera_dma_num_dwords)) {
				bk_ptr->dma_status.pass_simul = 0;
...
static int __init map_bars(struct altera_pcie_dma_bookkeep *bk_ptr,
			   struct pci_dev *dev)
{
	int i;
	for (i = 0; i < ALTERA_DMA_BAR_NUM; i++) {
		...
		if (!bk_ptr->bar[i]) {
			...
			dev_info(&dev->dev,
				 "BAR[%d] mapped to 0x%p, length %lu", i,
				 bk_ptr->bar[i],
				 (long unsigned int)bk_ptr->bar_length[i]);
	}
	return 0;
}

static void unmap_bars(struct altera_pcie_dma_bookkeep *bk_ptr,
		       struct pci_dev *dev)
{
	int i;
	for (i = 0; i < ALTERA_DMA_BAR_NUM; i++) {
...
static int __init altera_pci_probe(struct pci_dev *dev,
				   const struct pci_device_id *id)
{
	...
	if (rc) {
		dev_info(&dev->dev, "Could not request IRQ #%d",
			 bk_ptr->irq_line);
		bk_ptr->irq_line = -1;
		goto err_irq;
	} else {
	...
	bk_ptr->table_rd_cpu_virt_addr =
		((struct dma_desc_table *)pci_alloc_consistent(
			dev, sizeof(struct dma_desc_table),
			&bk_ptr->table_rd_bus_addr));
	bk_ptr->lite_table_rd_cpu_virt_addr =
		((struct lite_dma_desc_table *)pci_alloc_consistent(
			dev, sizeof(struct lite_dma_desc_table),
			&bk_ptr->lite_table_rd_bus_addr));
	if (!bk_ptr->table_rd_cpu_virt_addr ||
	    !bk_ptr->lite_table_rd_cpu_virt_addr) {
		rc = -ENOMEM;
		goto err_rd_table;
	}
	...
	if (!bk_ptr->table_wr_cpu_virt_addr ||
	    !bk_ptr->lite_table_wr_cpu_virt_addr) {
		rc = -ENOMEM;
		goto err_wr_table;
	}
	bk_ptr->numpages = (PAGE_SIZE >= MAX_NUM_DWORDS * 4) ?
				   1 :
				   (int)((MAX_NUM_DWORDS * 4) / PAGE_SIZE);
	bk_ptr->rp_rd_buffer_virt_addr =
		pci_alloc_consistent(dev, PAGE_SIZE * bk_ptr->numpages,
...
err_wr_buffer:
	...
	pci_free_consistent(dev, PAGE_SIZE * bk_ptr->numpages,
			    bk_ptr->rp_rd_buffer_virt_addr,
			    bk_ptr->rp_rd_buffer_bus_addr);
err_rd_buffer:
	dev_err(&dev->dev, "goto err_rd_buffer");
	pci_free_consistent(dev, sizeof(struct dma_desc_table),
			    bk_ptr->table_wr_cpu_virt_addr,
			    bk_ptr->table_wr_bus_addr);
err_wr_table:
...
static void __exit altera_pci_remove(struct pci_dev *dev)
{
	...
	pci_free_consistent(dev, sizeof(struct dma_desc_table),
			    bk_ptr->table_rd_cpu_virt_addr,
			    bk_ptr->table_rd_bus_addr);
	pci_free_consistent(dev, sizeof(struct lite_dma_desc_table),
			    bk_ptr->lite_table_rd_cpu_virt_addr,
			    bk_ptr->lite_table_rd_bus_addr);
	pci_free_consistent(dev, sizeof(struct dma_desc_table),
			    bk_ptr->table_wr_cpu_virt_addr,
...
static struct pci_device_id pci_ids[] = { { PCI_DEVICE(ALTERA_DMA_VID,
						       ALTERA_DMA_DID) },
					  { 0 } };
...
static int __init altera_dma_init(void)
{
	...
	if (rc) {
		printk(KERN_ERR ALTERA_DMA_DRIVER_NAME
	...
	/* uint32_t dma_vaddr; */
	/* dma_vaddr = dma_alloc_coherent(NULL, 512, dma_handle, GFP_KERNEL); */
	/* if (dma_vaddr == NULL) { */
	/* 	goto exit; */
	/* } */
	/* printk(KERN_ERR ALTERA_DMA_DRIVER_NAME ": Allocated 512 bytes at %i\n", dma_handle); */
...
*/
static int diff_timeval(ktime_t *result, ktime_t *t2, ktime_t *t1)
{
	// long int diff = (t2->tv_usec + 1000000 * t2->tv_sec) - (t1->tv_usec + 1000000 * t1->tv_sec);
	// result->tv_sec = diff / 1000000;
	// result->tv_usec = diff % 1000000;
	long int diff = ktime_us_delta(*t2, *t1);
	*result = ktime_sub((*t2), (*t1));
	return (diff < 0);
}
